function [mi,p,MI] = modulation_index_randperm(Vlo, Vhi,pvals)

%compute the modulation index of Vlo, Vhi where Phi_low and A_hi have been
%randomly sampled with replacement (in the same manner)

  phi=angle(hilbert(Vlo));%Compute phase of low freq signal.
  amp=abs(hilbert(Vhi));	%Compute amplitude of high freq signal.
  
  rp = ceil(rand(1,length(Vlo))*length(Vlo));
  phi = phi(rp);
  amp = amp(rp);
