...
PAC_mod = 0:0.05:.5;
J = 1000;
rcfc = zeros(1,J); rpac = zeros(1,J); rpac_new = zeros(1,J);
p_rcfc = zeros(1,J); p_rpac = zeros(1,J); p_rpac_new = zeros(1,J);
mi = zeros(1,J); p_mi = zeros(1,J);
...
for j = 1:J
    ...
    rcfc(j) = XX1.rcfc; rpac(j) = XX1.rpac; rpac_new(j) = XX1.rpac_new;
    p_rcfc(j) = P1.rcfc; p_rpac(j) = P1.rpac; p_rpac_new(j) = P1.rpac_new;
    [MI1,p_MI1] = modulation_index(Vlo,Vhi,'pvals');
    mi(j) = MI1; p_mi(j) = p_MI1;

end

strname = ['R_MI_Comparison_Weak_Coupling_',id];
save(strname)